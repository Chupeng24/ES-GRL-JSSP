...
path = './SavedNetwork/{}.pth'.format(str(N_JOBS_N) + '_' + str(N_MACHINES_N) + '_' + str(LOW) + '_' + str(HIGH) + '_2021-12-13-20-13-55')
...
def t(dataset):
    ...
    for i, data in enumerate(dataset):
        fea, adj, _, _, candidate, undoable_mask, done = env.reset(processing_time_matrix=data[0], machine_matrix=data[1],sched_ratio=0)
    ...
    t2 = time.time()
    print(t2 - t1)
    for item in makespans_list:
        print(item)
    print(np.mean(makespans_list))
