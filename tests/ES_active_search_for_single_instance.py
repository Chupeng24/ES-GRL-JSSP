...
def evaluate(env, agent, seed, optimal_value):
    # np.random.seed(configs.np_seed_train)
    # random.seed(configs.python_seed)
    # n_m = np.random.randint(5, 10)
    # n_j = np.random.randint(n_m, 10)
...
def eval_with_noise(env, agent, actor_net_noise, gnn_net_noise, seed, optimal_value):
    actor_net = agent.policy.actor
    gnn_net = agent.policy.feature_extract
    ...
    for p, p_n in zip(gnn_net.parameters(), gnn_net_noise):
        p.data += NOISE_STD * p_n
    r, s = evaluate(env, agent, seed, optimal_value)
...
def main(torch_seed, other_seed):
    df = pd.read_csv(f'optim_makespan_of_random_gen_ins_1800_200_seed-{other_seed}.csv', index_col=0)
    ...
    comment = f"ES, 5 fea, torch_seed=f'{torch_seed}', other_seed=f'{other_seed}', exact fitness func"
    ...
    for i_update in range(1600):
        ...
        for idx in range(MAX_BATCH_EPISODES):
            ...
            reward, steps = eval_with_noise(env, ppo, actor_net_pos, gnn_net_pos, seed_array[seed_idx], optim_list[seed_idx])
            seed_idx = seed_idx + 1
            batch_reward.append(reward)
            batch_steps += steps
            reward, steps = eval_with_noise(env, ppo, actor_net_neg, gnn_net_neg, seed_array[seed_idx], optim_list[seed_idx])
            seed_idx = seed_idx + 1
            batch_reward.append(reward)
            batch_steps += steps
