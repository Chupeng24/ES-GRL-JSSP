...
writer = SummaryWriter(log_dir=f'runs/{configs.n_j}x{configs.n_m}-lr-{configs.lr}-entro-{configs.entloss_coef}'+ TIMESTAMP)
...
class PPO:
    ...
    def update(self, memory, n_tasks, g_pool):
        ...
        for _ in range(self.k_epochs):
            ...
            advantages = rewards - vals.detach().view(-1)
        ...
        if configs.decayflag:
            self.scheduler.step()
        return loss.item(), v_loss.item(), -ent_loss.item()
