...
class JobManager:
                        ...
                        if not_end_cond:  # Construct forward flow conjunctive edges only
                            ...
                            for disj_op in op.disjunctive_ops:
                                if disj_op.x['type'] != DONE_NODE_SIG:
                                    g.add_edge(op.id, disj_op.id, type=DISJUNCTIVE_TYPE, direction=NO_DI)
    ...
    def draw_gantt_chart(self, path, benchmark_name, max_x):
        ...
        # temp['Start'] = 78
        # temp['Finish'] = 82
        # temp['Resource'] = "ZJob cannot be processed"
        # gantt_info.append(temp)
        color = OrderedDict()
        random.seed(200)
        for g in gantt_info:
        ...
        #     job_idx = "Job" + str(j)
        #     color[job_idx] = rgb
        # color["ZJob cannot be processed"] = 'rgb({}, {}, {})'.format(0, 0, 0)
